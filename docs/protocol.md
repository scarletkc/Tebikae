...
- `SyncState.pullRetryAt` / `pullRetryAttempts` 记录拉取重试，`pullRetryFull` 保留失败全量扫描的要求，重启后不能改用增量游标继续。只有完整成功的一轮拉取才推进游标并清除这些字段。分页中断不会确认远端删除；认证等永久性拉取错误不会触发新的重试唤醒。
...
每个已启动的引擎只维护一个重试唤醒计时器，根据当前数据库记录重新计算下一次时间。一次确认后仍有更新的本地输入时，继续按写入节流安排剩余 pending 操作。只读连接允许未知创建的只读查找，但不安排写入。后台标签页和离线状态暂停重试；回到前台或恢复网络后重新安排。停止会话会移除监听器与计时器，旧会话不能因延迟唤醒继续发送。计时器不是后台服务：关闭网页后不会同步，下次打开时从数据库恢复。
