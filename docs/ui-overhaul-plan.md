...
- 强调色从偏灰的 `#2f6f4f` 改为 `#1d7a52`，深色模式从 `#6fbf8f` 改为 `#4cc38a`（`theme.css`）。
- 品牌图标改为“右下角折起的图块 + 粗 T”（`public/icon.svg`）。PWA 另有一张白底的 `icon-maskable-512.png`，SVG 不再直接作为 maskable 图标。
- 搜索框：`TextContextMenu` 新增 `className`，外层不再挡住输入框的 `flex-1`，清除和筛选按钮回到右侧。
- 侧栏标签行：计数与“笔记”计数对齐，整行统一悬停和选中底色。
- 编辑器：删除模式说明文字（D16 收尾），工具栏和模式切换放在同一行；源码模式去掉聚焦描边，内边距与可视化模式一致，高度随内容增长；`--font-mono` 补上中文字体回退。
...
| `accent`                                                                            | `--accent`         | 主按钮、链接、选中     | `#1d7a52`（5.3:1）  | `#4cc38a`（7.9:1）  |
